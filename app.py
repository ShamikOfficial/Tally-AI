...
def multi_parent_matching(
    ...
    chain_penalty_weight=1000.0,
    max_date_diff_days=7,
    max_parent_group_size=1
):
    used_children = set()
    used_parents = set()
    matches = []
...
if uploaded_file is not None:
    ...
    with st.spinner("Reading and processing file..."):
        ...
        for required_col, default_col in default_mappings.items():
            ...
            column_mappings[required_col] = st.selectbox(
        ...
        for i, filter_config in enumerate(st.session_state.filters):
            ...
            with col2:
                ...
                if filter_config["column"] is not None:
                    unique_values = df[filter_config["column"]].unique()
        ...
        if st.button("Run Matching Process"):
            with st.spinner("Processing matches..."):
                ...
                for idx, store_id in enumerate(filtered_df['Store ID'].unique()):
                    ...
                    # Create children dictionary with global indices
                    children_dict = {}
                    for _, row in valid_child_df.iterrows():
                        child_id = f"c_{current_child_idx}"
                        global_child_id_to_idx[child_id] = current_child_idx
                        children_dict[child_id] = (row["Tender Amount"], row["Sales Date"])
                        current_child_idx += 1
                ...
                # Process results
                # Create a results dataframe with just the matching information
                matches_df = pd.DataFrame(index=filtered_df.index)
                matches_df["Matched Parent ID"] = None
                matches_df["Matched Parent Amount"] = None
                matches_df["Effective Parent Amount"] = None
                matches_df["Effective Parent-Child Difference"] = None
                
                # Create a mapping of filtered indices to original indices
                filtered_to_original = {i: idx for i, idx in enumerate(filtered_df.index)}
                ...
                for parent_ids, child_ids, parent_amt, combo_amt, amt_diff, match_date in match_groups_all:
                    ...
                    for i, child_id in enumerate(child_ids):
                        ...
                        filtered_idx = global_child_id_to_idx[child_id]
                        original_idx = filtered_to_original[filtered_idx]
                        tender_amt = filtered_df.iloc[filtered_idx]["Tender Amount"]
                        
                        matches_df.loc[original_idx, "Matched Parent ID"] = parent_id_str
                        matches_df.loc[original_idx, "Matched Parent Amount"] = parent_amt
                        
                        if i == len(child_ids) - 1:
                            eff_amt = parent_amt - running_amt
                        else:
                            eff_amt = tender_amt
                            running_amt += tender_amt
                        
                        matches_df.loc[original_idx, "Effective Parent Amount"] = eff_amt
                        matches_df.loc[original_idx, "Effective Parent-Child Difference"] = eff_amt - tender_amt
                
                # Left join the matches with original data
                results_df = df.join(matches_df)
                ...
                # Display results table with all original data
                ...
                with col2:
                    ...
                    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                        results_df.to_excel(writer, index=True, sheet_name='Matching Results')
